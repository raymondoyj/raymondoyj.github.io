...
subtitle:   "XXL-JOB Documentation"
date:       2019-07-19 
author:     "xuxueli"
...
![登录后运行界面](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_6yC0.png "登录后运行界面")
...
本示例以新建一个 “GLUE模式(Java)” 运行模式的任务为例。更多有关任务的详细配置，请查看“3. 任务详解”。
...
![调度中心-任务管理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_o8HQ.png "调度中心-任务管理")

![新建任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_ZAsz.png "新建任务")


#### 2.5.2 步骤二：“GLUE模式(Java)” 任务开发：
...
![示例任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Fgql.png "示例任务")

![GLUE编辑器开发界面](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_dNUJ.png "GLUE编辑器开发界面")
...
![任务日志](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_inc8.png "任务日志")

在日志控制台，可以Rolling方式实时查看任务在执行器一侧运行输出的日志信息，实时监控任务进度；

![日志控制台](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_eYrv.png "日志控制台")
...
![IJobHandler](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_oLlM.png "IJobHandler")

#### 3.2.2 步骤二：调度中心，新建调度任务
参考上文“配置属性详细说明”对新建的任务进行参数配置，运行模式选中 "BEAN模式"，JobHandler属性填写任务注解“@JobHandler”中定义的值；

![参数配置](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_ZAsz.png "参数配置")
...
#### 3.3.1 步骤一：调度中心，新建调度任务：
参考上文“配置属性详细说明”对新建的任务进行参数配置，运行模式选中 "GLUE模式(Java)"；

![新建调度任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_tJOq.png "新建调度任务")
...
![开发任务代码](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_dNUJ.png "开发任务代码")

### 3.4 GLUE模式(Shell)

#### 3.4.1 步骤一：调度中心，新建调度任务   
...
![开发任务代码](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_iUw0.png "开发任务代码")

### 3.5 GLUE模式(Python)

#### 3.5.1 步骤一：调度中心，新建调度任务   
...
![开发任务代码](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_BPLG.png "开发任务代码")

### 3.6 GLUE模式(NodeJS)

#### 3.6.1 步骤一：调度中心，新建调度任务   
...
### 4.1 配置执行器
点击进入"执行器管理"界面, 如下图:
![执行器管理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Hr2T.png "执行器管理")
...
![新增执行器](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_V3vF.png "新增执行器")
...
### 4.3 新建任务
进入任务管理界面，点击“新增任务”按钮，在弹出的“新增任务”界面配置任务属性后保存即可。详情页参考章节 "3. 任务详解"。
...
### 4.6 启动/停止任务
可对任务进行“启动”和“停止”操作。
需要注意的是，此处的启动/停止仅针对任务的后续调度触发行为，不会影响到已经触发的调度任务，如需终止已经触发的调度任务，可查看“4.10 终止运行中的任务”

![启动/停止任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_ZAhX.png "启动/停止任务")

### 4.7 手动触发一次调度
点击“执行”按钮，可手动触发一次任务调度，不影响原有调度规则。

![“执行”按钮](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_ZAhX.png "“执行”按钮")

### 4.8 查看调度日志
点击“日志”按钮，可以查看任务历史调度日志。在历史调入日志界面可查看每次任务调度的调度结果、执行结果等，点击“执行日志”按钮可查看执行器完整日志。

![执行日志](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_ZAhX.png "执行日志")

![执行日志](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_UDSo.png "执行日志")

    调度时间："调度中心"触发本次调度并向"执行器"发送任务执行信号的时间；
    调度结果："调度中心"触发本次调度的结果，200表示成功，500或其他表示失败；
    调度备注："调度中心"触发本次调度的日志信息；
    执行器地址：本次任务执行的机器地址
    运行模式：触发调度时任务的运行模式，运行模式可参考章节 "3. 任务详解"；
    任务参数：本地任务执行的入参
    执行时间："执行器"中本次任务执行结束后回调的时间；
    执行结果："执行器"中本次任务执行的结果，200表示成功，500或其他表示失败；
    执行备注："执行器"中本次任务执行的日志信息；
    操作：
        "执行日志"按钮：点击可查看本地任务执行的详细日志信息；详见“4.9 查看执行日志”；
        "终止任务"按钮：点击可终止本地调度对应执行器上本任务的执行线程，包括未执行的阻塞任务一并被终止；

### 4.9 查看执行日志
点击执行日志右侧的 “执行日志” 按钮，可跳转至执行日志界面，可以查看业务代码中打印的完整日志，如下图；

![执行日志](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_tvGI.png "执行日志")

### 4.10 终止运行中的任务
仅针对执行中的任务。
在任务日志界面，点击右侧的“终止任务”按钮，将会向本次任务对应的执行器发送任务终止请求，将会终止掉本次任务，同时会清空掉整个任务执行队列。

![终止任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_hIci.png "终止任务")
...
![日志清理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Ypik.png "日志清理")

![日志清理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_EB65.png "日志清理")

### 4.12 删除任务
点击删除按钮，可以删除对应任务。

![删除任务](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Z9Qr.png "删除任务")
...
![用户管理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_1001.png "用户管理")

![用户管理](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_1002.png "用户管理")
...
![架构图](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Qohm.png "架构图")
...
![调度备注](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_jrdI.png "调度备注")
...
- 执行日志：任务执行过程中，业务代码中打印的完整执行日志，见“4.9 查看执行日志”；
...
![任务日志界面](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_Wb2o.png "任务日志界面")

![执行备注](https://raw.githubusercontent.com/xuxueli/xxl-job/master/doc/images/img_jOAU.png "执行备注")
...
需要注意的是，任务超时中断时与任务终止机制（可查看“4.10 终止运行中的任务”）类似，也是通过 "interrupt" 中断任务，因此业务代码需要将 "InterruptedException" 外抛，否则功能不可用。
